...
function find_active_keybindings(key)
	...
	-- Build the result list from the DEDUPED map (one bind per key) so superseded/shadowed
	-- bindings (e.g. an overridden mpv default still present in input-bindings) don't show twice
	-- in the keybinds menu. (Upstream appended to active_table on every win, keeping shadows.)
	local active_table = {}
	for _, bind in pairs(active_map) do active_table[#active_table + 1] = bind end

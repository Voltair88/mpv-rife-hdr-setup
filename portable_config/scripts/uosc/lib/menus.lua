...
-- ─── Key bindings menu: categories / friendly names / hide-list ──────────────────────────────
-- Niche bindings to omit from the menu (they still work as keys). Edit to taste.
local KEYBINDS_HIDE = {
	'video%-align', 'video%-scale', 'sub%-ass', 'ass%-override', 'secondary%-sub',
	'revert%-seek mark', 'set playlist%-pos',
}

-- Category by keyword/pattern. ORDER MATTERS: specific entries before generic ones.
local KEYBINDS_CATEGORIES = {
	{'video%-zoom', 'Zoom & Pan'}, {'video%-pan', 'Zoom & Pan'}, {'video%-rotate', 'Zoom & Pan'},
	{'video%-scale', 'Zoom & Pan'}, {'video%-align', 'Zoom & Pan'}, {'panscan', 'Zoom & Pan'}, {'zoom', 'Zoom & Pan'},
	{'contrast', 'Video & Color'}, {'brightness', 'Video & Color'}, {'gamma', 'Video & Color'},
	{'saturation', 'Video & Color'}, {'hue', 'Video & Color'}, {'deband', 'Video & Color'},
	{'deinterlace', 'Video & Color'}, {'aspect', 'Video & Color'}, {'cycle video', 'Video & Color'},
	{'edition', 'Video & Color'},
	{'sub', 'Subtitles'},
	{'audio', 'Audio'}, {'volume', 'Audio'}, {'mute', 'Audio'},
	{'seek', 'Seeking'},
	{'chapter', 'Chapters & Playlist'}, {'playlist', 'Chapters & Playlist'},
	{'pause', 'Playback'}, {'speed', 'Playback'}, {'frame%-step', 'Playback'}, {'frame%-back%-step', 'Playback'},
	{'fullscreen', 'Window & View'}, {'window%-scale', 'Window & View'}, {'ontop', 'Window & View'},
	{'osd', 'Window & View'}, {'border', 'Window & View'},
	{'screenshot', 'Screenshots & Info'}, {'stats', 'Screenshots & Info'}, {'show%-progress', 'Screenshots & Info'},
	{'show%-text', 'Screenshots & Info'},
	{'quit', 'Misc'}, {'ab%-loop', 'Misc'}, {'loop%-file', 'Misc'}, {'hwdec', 'Misc'},
	{'loadfile', 'Misc'}, {'toggle_rife', 'Misc'},
}

-- Fixed display order of the top-level category submenus.
local KEYBINDS_CATEGORY_ORDER = {
	'Playback', 'Seeking', 'Chapters & Playlist', 'Audio', 'Subtitles',
	'Video & Color', 'Zoom & Pan', 'Window & View', 'Screenshots & Info', 'Misc', 'Other',
}

local function keybind_category(cmd)
	for _, entry in ipairs(KEYBINDS_CATEGORIES) do
		if cmd:find(entry[1]) then return entry[2] end
	end
	return 'Other'
end

local function keybind_hidden(cmd)
	for _, pat in ipairs(KEYBINDS_HIDE) do
		if cmd:find(pat) then return true end
	end
	return false
end

local function kb_signed(v) local n = tonumber(v); return (n >= 0 and '+' or '') .. n end
local function kb_ms(v) local n = math.floor(tonumber(v) * 1000 + 0.5); return (n >= 0 and '+' or '') .. n .. 'ms' end

-- Ordered friendly-name rules: {pattern, label|function(capture)}. First match wins; nil -> raw cmd.
local KEYBINDS_FRIENDLY = {
	{'^cycle pause$', 'Play / Pause'}, {'^set pause yes$', 'Pause'}, {'^set pause no$', 'Unpause'},
	{'^frame%-step$', 'Frame step forward'}, {'^frame%-back%-step$', 'Frame step back'},
	{'^multiply speed ([%d%./]+)$', function(v) return 'Speed × ' .. v end}, {'^set speed 1%.0$', 'Reset speed'},
	{'^no%-osd seek (%-?%d+) exact', function(v) return 'Seek ' .. kb_signed(v) .. 's (exact)' end},
	{'^seek (%-?%d+) absolute', function(v) return 'Seek to ' .. v .. 's' end},
	{'^seek (%-?%d+)', function(v) return 'Seek ' .. kb_signed(v) .. 's' end},
	{'^revert%-seek', 'Undo seek'},
	{'^add chapter 1$', 'Next chapter'}, {'^add chapter %-1$', 'Previous chapter'},
	{'^playlist%-next', 'Next file'}, {'^playlist%-prev', 'Previous file'},
	{'^add volume (%-?%d+)', function(v) return 'Volume ' .. kb_signed(v) end},
	{'^cycle mute$', 'Mute'}, {'^cycle audio$', 'Cycle audio track'},
	{'^add audio%-delay (%-?[%d%.]+)', function(v) return 'Audio delay ' .. kb_ms(v) end},
	{'^cycle sub%-visibility$', 'Toggle subtitles'},
	{'^cycle sub down$', 'Cycle subtitle track (back)'}, {'^cycle sub$', 'Cycle subtitle track'},
	{'^add sub%-delay ([%+%-]?[%d%.]+)', function(v) return 'Subtitle delay ' .. kb_ms(v) end},
	{'^add sub%-scale ([%+%-]?[%d%.]+)', function(v) return 'Subtitle size ' .. kb_signed(v) end},
	{'^add sub%-pos ([%+%-]?%d+)', function(v) return 'Subtitle position ' .. kb_signed(v) end},
	{'^no%-osd sub%-seek (%-?%d+)', function(v) return 'Subtitle seek ' .. kb_signed(v) end},
	{'^sub%-step (%-?%d+)', function(v) return 'Subtitle step ' .. kb_signed(v) end},
	{'^add contrast (%-?%d+)', function(v) return 'Contrast ' .. kb_signed(v) end},
	{'^add brightness (%-?%d+)', function(v) return 'Brightness ' .. kb_signed(v) end},
	{'^add gamma (%-?%d+)', function(v) return 'Gamma ' .. kb_signed(v) end},
	{'^add saturation (%-?%d+)', function(v) return 'Saturation ' .. kb_signed(v) end},
	{'^cycle deband$', 'Toggle debanding'}, {'^cycle deinterlace$', 'Toggle deinterlace'},
	{'^cycle video$', 'Cycle video track'}, {'^cycle edition$', 'Cycle edition'},
	{'^add video%-zoom (%-?[%d%.]+)', function(v) return 'Zoom ' .. kb_signed(v) end}, {'^set video%-zoom 0$', 'Reset zoom'},
	{'^add video%-rotate (%-?%d+)', function(v) return 'Rotate ' .. kb_signed(v) end}, {'^set video%-rotate 0$', 'Reset rotation'},
	{'^cycle fullscreen$', 'Toggle fullscreen'}, {'^set fullscreen no$', 'Leave fullscreen'},
	{'^set window%-scale ([%d%.]+)', function(v) return 'Window scale ' .. v end},
	{'^cycle ontop$', 'Toggle always-on-top'},
	{'^add panscan ([%+%-]?[%d%.]+)', function(v) return 'Panscan ' .. kb_signed(v) end},
	{'^cycle%-values video%-aspect%-override', 'Cycle aspect ratio'}, {'^cycle%-values osd%-level', 'Cycle OSD'},
	{'^screenshot video', 'Screenshot (no subs)'}, {'^screenshot window', 'Screenshot (with OSD)'},
	{'^screenshot each%-frame', 'Screenshot every frame'}, {'^screenshot', 'Screenshot'},
	{'^show%-progress', 'Show progress'}, {'^script%-binding stats/', 'Statistics'},
	{'^quit%-watch%-later', 'Quit (remember position)'}, {'^quit', 'Quit'},
	{'^cycle%-values loop%-file', 'Toggle loop'}, {'^ab%-loop', 'A-B loop'},
	{'^cycle%-values hwdec', 'Toggle hardware decoding'}, {'^script%-binding toggle_rife', 'Toggle RIFE'},
}

local function keybind_friendly(cmd)
	for _, rule in ipairs(KEYBINDS_FRIENDLY) do
		local cap = cmd:match(rule[1])
		if cap ~= nil then
			local label = rule[2]
			if type(label) == 'function' then return label(cap) end
			return label
		end
	end
	return nil
...
function get_keybinds_items()
	...
	-- Group by command (one row per action, listing all its keys); skip hidden binds.
	local by_cmd = {} -- cmd -> { keys = {human...}, seen = {human=true} }
	local order = {}
	for _, bind in pairs(binds_dump) do
		local cmd = bind.cmd
		if cmd and cmd ~= '' and not keybind_hidden(cmd) then
			local group = by_cmd[cmd]
			if not group then group = {keys = {}, seen = {}}; by_cmd[cmd] = group; order[#order + 1] = cmd end
			local human = keybind_to_human(bind.key) or bind.key
			if human and human ~= '' and not group.seen[human] then
				group.seen[human] = true
				group.keys[#group.keys + 1] = human
			end
		end
	end

	-- Build leaf items, bucketed by category.
	local buckets = {} -- category -> { {title, hint, value}, ... }
	for _, cmd in ipairs(order) do
		local keys = by_cmd[cmd].keys
		local cat = keybind_category(cmd)
		buckets[cat] = buckets[cat] or {}
		local leaves = buckets[cat]
		leaves[#leaves + 1] = {
			title = keybind_friendly(cmd) or cmd,
			hint = #keys > 0 and table.concat(keys, ' / ') or nil,
			value = cmd,
		}
	end

	-- Emit category submenus in fixed order (skip empties); sort leaves within each.
	local items = {}
	local emitted = {}
	local function emit(cat)
		local leaves = buckets[cat]
		if not leaves or #leaves == 0 or emitted[cat] then return end
		emitted[cat] = true
		table.sort(leaves, function(a, b) return a.title < b.title end)
		items[#items + 1] = {title = cat, hint = tostring(#leaves), items = leaves}
	end
	for _, cat in ipairs(KEYBINDS_CATEGORY_ORDER) do emit(cat) end
	for cat in pairs(buckets) do emit(cat) end -- safety: any unlisted category

...
function subprocess(args, async, callback)
    ...
    if not pre_0_30_0 then
        if async then
            -- NOTE: do NOT restrict env to PATH-only. The Vulkan loader (needed by the
            -- libplacebo HDR tone-map filter) requires SystemRoot etc. to find the GPU driver;
            -- stripping the environment to PATH causes VK_ERROR_INCOMPATIBLE_DRIVER -> the
            -- filter fails -> black/empty thumbnails. Omitting env inherits the full parent
            -- environment, so libplacebo can initialize the GPU.
            return mp.command_native_async(
            { name = "subprocess", playback_only = true, args = args }, callback)
        else
...
local function vf_string(filters, full)
    ...
    -- Downscale to thumbnail size BEFORE the HDR tone-map. Tone-mapping the full 2160p
    -- float frame (original order) takes >60s/frame and effectively hangs the worker, which
    -- forced tone_mapping=no -> HDR thumbnails came out near-black. Tone-mapping a ~200x84
    -- frame instead is effectively free and produces correctly tone-mapped, visible thumbs.
    if full then
    ...
    if (full and options.tone_mapping ~= "no") or options.tone_mapping == "auto" then
        if properties["video-params"] and properties["video-params"]["primaries"] == "bt.2020" then
            ...
            -- Original used zscale, which HANGS on Dolby Vision content (proven: even a bare
            -- zscale transfer-convert never returns). libplacebo tone-maps on the GPU and
            -- renders in ~2.8s cold / fast warm, producing visible HDR thumbnails.
            vf = vf .. "libplacebo=tonemapping=" .. tone_mapping .. ":colorspace=bt709:color_primaries=bt709:color_trc=bt709,"
        end
    end

    if full then
        vf = vf .. "pad=w=" .. effective_w .. ":h=" .. effective_h .. ":x=-1:y=-1,format=bgra"
    end
...
local function info(w, h)
    ...
    -- Paused-only display: report "disabled" to the UI while the video is PLAYING, so uosc draws
    -- no thumbnail box and stops sending hover requests (no black/empty thumb during playback,
    -- which can't generate correctly anyway while RIFE owns the decode path). The internal
    -- `disabled` flag is left unchanged so the worker still spawns normally when paused.
    local ui_disabled = disabled or (mp.get_property_native("pause") == false)
    local json, err = mp.utils.format_json({ width = w * options.scale_factor, height = h * options.scale_factor, scale_factor =
    options.scale_factor, disabled = ui_disabled, available = true, socket = options.socket, thumbnail = options.thumbnail, overlay_id =
    options.overlay_id })
    if pre_0_30_0 then
...
local function spawn(time)
    ...
    -- NOTE: the worker spawn (CreateProcessW) can fail with ERROR_INVALID_PARAMETER (87) while
    -- RIFE is actively inferring during playback (a handle-inheritance race); it succeeds when
    -- RIFE is idle (i.e. when the video is paused). We previously auto-paused playback to dodge
    -- this, but that briefly froze the video on first hover, so it was removed by request.
    -- Result: thumbnails generate reliably while paused; hovering during playback may fail.
    ...
    subprocess(args, true,
        function(success, result)
            if spawn_waiting and (success == false or (result.status ~= 0 and result.status ~= -2)) then
                ...
                -- The worker spawn (CreateProcessW, error 87) only fails while RIFE is actively
                -- inferring during playback; it succeeds once paused. That case is expected and
                -- transient, so don't disable tone-mapping or flash the "cannot create subprocess"
                -- banner for it -- a genuine failure (bad mpv_path / missing mpv) would also fail
                -- while paused / idle, where we still surface it.
                local playback_race = mp.get_property_native("pause") == false
                if not playback_race then options.tone_mapping = "no" end
                if not spawn_working and not playback_race then -- notify users of required configuration
...
local function thumb(time, r_x, r_y, script)
    if disabled then return end
    -- Paused-only: ignore hover requests while playing (belt-and-suspenders; uosc already stops
    -- sending these because info() reports disabled during playback).
...
-- Paused-only display: when playback resumes, clear any shown thumbnail; on every pause toggle,
-- re-send info() so uosc enables the thumbnail box only while paused.
mp.observe_property("pause", "native", function(_, paused)
    if paused == false then clear() end
    info(real_w or effective_w, real_h or effective_h)
end)
